...
library(dplyr)
# 设置工作目录为包含数据文件的目录
setwd("C:/Users/lyr/Desktop/TCGA/gene_exp/liver/")
# 搜索所有子文件夹中的TSV文件
# 找到所有子文件夹中的TSV文件
...
head(final_data)
# 保存合并后的数据为CSV文件
write.csv(final_data, "C:/Users/lyr/Desktop/TCGA/gene_exp/liver/liver_TCGA_gene_exp.csv", row.names = FALSE,quote = FALSE)

#提取分组信息
gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest_stage1.2024-09-26.txt",sep = "\t")
gro1 <- gro[, c("filename"), drop = FALSE]

gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest_stage2.2024-09-26.txt",sep = "\t")
gro2 <- gro[, c("filename"), drop = FALSE]

gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest_stage3.2024-09-26.txt",sep = "\t")
gro3 <- gro[, c("filename"), drop = FALSE]

gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest_stage4.2024-09-26.txt",sep = "\t")
gro4 <- gro[, c("filename"), drop = FALSE]

gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest_control.2024-09-26.txt",sep = "\t")
gro5 <- gro[, c("filename"), drop = FALSE]
...
all_data <- rbind(gro1, gro2,gro3,gro4,gro5)

gro<-read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/manifest/liver/gdc_manifest.2024-09-26.txt",sep = "\t")
filter_vector<-gro$filename
all_data <- all_data %>% filter(filename %in% filter_vector)

# 显示前几行数据
head(all_data)

write.csv(all_data,"C:/Users/lyr/Desktop/TCGA/liver_sample_info.csv",row.names = FALSE)

library(limma)
library(edgeR)

# 导入表达矩阵，假设数据已标准化为FPKM/TPM或为counts数据
expression_data <- read.csv("C:/Users/lyr/Desktop/TCGA/gene_exp/liver/liver_TCGA_gene_exp.csv", row.names = 1)
all_data<- read.csv("C:/Users/lyr/Desktop/TCGA/liver_sample_info.csv", row.names = 1)
...
# 导入分组信息
group_info <- read.csv("C:/Users/lyr/Desktop/TCGA/liver_sample_info.csv")  # 包含样本名称和分组信息
...
p <- ggplot(expr_long, aes(x = Group, y = LogExpression, fill = Group)) +
  ...
  geom_signif(comparisons = list(c("stage1", "stage2"), 
                                 c("stage1", "control"), 
                                 c("stage2", "control")),
...
# 添加显著性标记
figure4 <- p + geom_signif(comparisons = list(c("stage1", "stage2"), 
                                              c("stage1", "control"), 
                                              c("stage2", "control")),
                           map_signif_level = TRUE,
                           step_increase = 0.1)
...
heatmap <- Heatmap(
  ...
  top_annotation = HeatmapAnnotation(
    df = annotation_col,
    col = list(Group = c(control = "lightgreen",stage1 = "lightblue", stage2 = "lightyellow"))
  )
)
